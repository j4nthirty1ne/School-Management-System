    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card className="cursor-pointer hover:shadow-lg transition-shadow dark:bg-[#1a1a1a] dark:border-gray-800" onClick={() => setOpenDialog('attendance')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium dark:text-white">Attendance</CardTitle>
              <svg
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow dark:bg-[#1a1a1a] dark:border-gray-800" onClick={() => setOpenDialog('grades')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium dark:text-white">Average Grade</CardTitle>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="none"
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow dark:bg-[#1a1a1a] dark:border-gray-800" onClick={() => setOpenDialog('assignments')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium dark:text-white">Assignments</CardTitle>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="none"
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow dark:bg-[#1a1a1a] dark:border-gray-800" onClick={() => setOpenDialog('classes')}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium dark:text-white">Classes</CardTitle>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="none"
          ...
          {/* Dialogs for details */}
          <Dialog open={openDialog === 'attendance'} onOpenChange={(val) => setOpenDialog(val ? 'attendance' : null)}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Attendance Details</DialogTitle>
                <DialogDescription>Your attendance history and trends</DialogDescription>
              </DialogHeader>
              <div className="mt-4 space-y-3">
                {dialogLoading ? (
                  <div className="text-sm text-gray-500 dark:text-gray-400">Loading...</div>
                ) : (
                  (attendanceData.length ? attendanceData : [{ date: '2025-10-01', status: 'Present' }, { date: '2025-10-02', status: 'Absent' }, { date: '2025-10-03', status: 'Present' }]).map((row: any, i: number) => (
                    <div key={i} className="flex justify-between p-3 border border-border rounded-md dark:bg-[#1a1a1a] dark:border-gray-800">
                      <div className="text-sm dark:text-white">{row.date}</div>
                      <div className={`text-sm font-medium ${row.status === 'present' || row.status === 'Present' ? 'text-green-600' : 'text-red-600'}`}>{row.status}</div>
                    </div>
                  ))
                )}
              </div>
          ...
          <Dialog open={openDialog === 'grades'} onOpenChange={(val) => setOpenDialog(val ? 'grades' : null)}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Grades Overview</DialogTitle>
                <DialogDescription>Your average grade and recent breakdown</DialogDescription>
              </DialogHeader>
                <div className="mt-4">
                  {dialogLoading ? (
                    <div className="text-sm text-gray-500 dark:text-gray-400">Loading...</div>
                    ...
                    <>
                      <div className="text-4xl font-bold dark:text-white">{gradesData.length ? (Math.round((gradesData.reduce((s: number, g: any) => s + (g.percentage || 0), 0) / gradesData.length) * 10) / 10 + '%') : 'A-'}</div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">{gradesData.length ? `Average ${Math.round((gradesData.reduce((s: number, g: any) => s + (g.percentage || 0), 0) / gradesData.length) * 10) / 10}% overall` : '85.3% overall'}</p>
                      <div className="mt-4 space-y-3">
                        {(gradesData.length ? gradesData : [{ subject: 'Mathematics', score: '92/100' }, { subject: 'Physics', score: '87/100' }, { subject: 'English', score: '88/100' }]).map((g: any, i: number) => (
                          <div key={i} className="flex justify-between p-3 border border-border rounded-md dark:bg-[#1a1a1a] dark:border-gray-800">
                            <div className="text-sm dark:text-white">{g.subject || g.subject_name || g.subject_id}</div>
                            <div className="text-sm font-medium dark:text-white">{g.score || (g.marks_obtained ? `${g.marks_obtained}/${g.total_marks}` : (g.percentage ? `${g.percentage}%` : 'N/A'))}</div>
                          </div>
                        ))}
                      </div>
                    </>
          ...
          <Dialog open={openDialog === 'assignments'} onOpenChange={(val) => setOpenDialog(val ? 'assignments' : null)}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Assignments</DialogTitle>
                <DialogDescription>List of your assignments and due dates</DialogDescription>
              </DialogHeader>
              <div className="mt-4 space-y-3">
                {dialogLoading ? (
                  <div className="text-sm text-gray-500 dark:text-gray-400">Loading...</div>
                ) : (
                  (assignmentsData.length ? assignmentsData : [{ title: 'Chemistry Lab Report', due: 'Oct 23', status: 'urgent' }, { title: 'History Essay', due: 'Oct 25', status: 'soon' }, { title: 'Math Problem Set', due: 'Oct 28', status: 'normal' }]).map((a: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 border border-border rounded-md dark:bg-[#1a1a1a] dark:border-gray-800">
                      <div>
                        <p className="font-medium dark:text-white">{a.title || a.assignment_title || a.name}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Due: {a.due_date || a.due || 'TBD'}</p>
                      </div>
                      <div>
                        <Badge variant={a.status === 'urgent' ? 'destructive' : 'outline'}>{a.due_date ? new Date(a.due_date).toLocaleDateString() : (a.due || '')}</Badge>
                      </div>
                    </div>
                  ))
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="dark:bg-[#1a1a1a] dark:border-gray-800">
            <CardHeader>
              ...
              <CardDescription className="dark:text-gray-400">Your latest exam and assignment scores</CardDescription>
            </CardHeader>
            <CardContent>
