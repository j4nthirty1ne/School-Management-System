...
import { ArrowRight, Mail, Gem, Lock, Eye, EyeOff, ArrowLeft, X, AlertCircle, PartyPopper, Loader, GraduationCap } from "lucide-react";
...
const modalSteps = [
    ...
    { message: "Loading your dashboard...", icon: <Loader className="w-12 h-12 text-primary animate-spin" /> },
...
const DefaultLogo = () => ( <div className="bg-primary text-primary-foreground rounded-md p-1.5"> <GraduationCap className="h-4 w-4" /> </div> );
...
export default function LoginPage({ logo = <DefaultLogo />, brandName = "School Management System" }: AuthComponentProps) {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok || !data?.success) {
        setModalErrorMessage(data?.error || "Login failed. Please check your credentials.");
...
useEffect(() => {
    if (showFullForm && authStep === 'password') setTimeout(() => passwordInputRef.current?.focus(), 500);
}, [showFullForm, authStep]);
        ...
        <div className={cn("flex w-full flex-1 h-full items-center justify-center bg-card", "relative overflow-hidden")}>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-full max-w-[400px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && !showFullForm && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                ...
                <form onSubmit={handleFinalSubmit} className="w-full max-w-[350px] space-y-6">
                     <AnimatePresence>
                        {!showFullForm && (
                                    ...
                                    <div className="glass-input-wrap w-full">
                                        <div className="glass-input h-12">
                                            ...
                                            <input 
                                                ...
                                                className="relative z-10 h-full w-full flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none pl-4 pr-12" 
                                            />
                                            <AnimatePresence>
                                                {isEmailValid && (
                                                    <motion.div 
                                                        ...
                                                        className="relative z-10 flex-shrink-0 overflow-hidden pr-1"
                                                    >
                                                        <GlassButton 
                                                            type="button" 
                        ...
                        {showFullForm && (
                            ...
                            >
                                <div className="text-center">
                                    <h2 className="text-3xl font-serif font-light tracking-tight text-foreground">Log In</h2>
                                </div>
                                <div className="border-2 border-border/30 rounded-2xl p-6 space-y-6 bg-card/30 backdrop-blur-sm w-[350px]">
                                        ...
                                        <div className="glass-input-wrap w-full">
                                            <div className="glass-input h-12">
                                                ...
                                                <input 
                                                    ...
                                                    value={email} 
                                                    onChange={(e) => setEmail(e.target.value)}
                                                    className="relative z-10 h-full w-full flex-grow bg-transparent text-foreground placeholder:text-foreground/60 focus:outline-none pl-4 pr-4" 
                                                />
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <label className="text-sm text-muted-foreground font-semibold">Password</label>
                                        </div>
                                        <div className="glass-input-wrap w-full">
                                            <div className="glass-input h-12">
                                                <span className="glass-input-text-area"></span>
                                                ...
                                                <button 
                                                    ...
                                                    className="relative z-10 text-foreground/80 hover:text-foreground transition-colors p-2 rounded-full mr-1"
                                                >
                                    ...
                                    <div className="space-y-4">
                                        <div className="relative w-full h-14 bg-muted/20 rounded-full overflow-hidden backdrop-blur-sm border border-border/30">
                                            <motion.div
                                                className="absolute inset-0 flex items-center justify-center pointer-events-none"
                                                animate={{ opacity: isUnlocked ? 0 : 1 }}
                                            >
                                                <span className="text-sm font-medium text-muted-foreground flex items-center gap-2">
                                                    <span>Swipe to Sign In</span>
                                            ...
                                            <motion.div
                                                ref={sliderRef}
                                                drag="x"
                                                dragConstraints={{ left: 0, right: sliderRef.current ? sliderRef.current.parentElement!.offsetWidth - 56 : 0 }}
                                                dragElastic={0}
                                                dragMomentum={false}
                                                onDrag={(e, info) => {
                                                    setIsDragging(true);
                                                    const parentWidth = sliderRef.current?.parentElement?.offsetWidth || 0;
                                                    const maxDrag = parentWidth - 56;
                                                    const progress = Math.min(Math.max(info.point.x / maxDrag, 0), 1);
                                                ...
                                                onDragEnd={(e, info) => {
                                                    ...
                                                    const parentWidth = sliderRef.current?.parentElement?.offsetWidth || 0;
                                                    const maxDrag = parentWidth - 56;
                                                    const progress = info.point.x / maxDrag;
                                                    
                                                    if (progress >= 0.85 && isEmailValid && isPasswordValid) {
                                                        setIsUnlocked(true);
                                                        setTimeout(() => {
                                                            setIsUnlocked(false);
                                                            setSliderPosition(0);
                                                ...
                                                className="absolute left-0 top-1/2 -translate-y-1/2 w-14 h-12 bg-primary rounded-full cursor-grab active:cursor-grabbing flex items-center justify-center shadow-lg"
                                                style={{
                                                    opacity: (!isEmailValid || !isPasswordValid) ? 0.5 : 1,
                                                    pointerEvents: (!isEmailValid || !isPasswordValid) ? 'none' : 'auto'
                                                }}
                                            >
                                ...
                                <button 
                                    ...
                                    className="w-full flex items-center justify-center gap-2 text-sm text-foreground/70 hover:text-foreground transition-colors mt-4"
                                >
